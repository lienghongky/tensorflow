...
TfLiteStatus NNAPIDelegate::Invoke(Interpreter* interpreter) {
  ...
  bool profiling = interpreter->GetProfiling();

  if (profiling) gettimeofday(&t0, NULL);
  ...
  if (profiling) gettimeofday(&t1, NULL);
  ...
  if (profiling) gettimeofday(&t2, NULL);
  ...
  if (profiling) {
    ...
    printf("nnapi model_creation \t = %f\n", (get_us(t1) - get_us(t0)));
    printf("nnapi i/o buffer \t = %f\n", (get_us(t2) - get_us(t1)));
    printf("nnapi running time \t = %f\n", (get_us(t3) - get_us(t2)));
  }
