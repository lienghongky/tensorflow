...
#include "tensorflow/compiler/aot/tests/test_graph_resnetv2.h"

using namespace std;
using namespace tensorflow::tfcompile;

void get_file_to_float(string fn, float bmp_float[][299][3])
{
  ifstream file(fn, ios::in|ios::binary|ios::ate);
  uint8_t bmp[299][299][3];

  if (file.is_open())
    file.read((char *)bmp, 299*299*3);
