...
class DecodeBmpOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    Decode(bmp_pixels, output->flat<uint8>().data(), header_size, width,
           abs(height), top_down, row_size);
  }

  uint8* Decode(uint8* const input, uint8* const output, const int header_Size,
                const int width, const int height, bool top_down, int row_size);
};
...
uint8* DecodeBmpOp::Decode(uint8* const input, uint8* const output,
                           const int header_Size, const int width,
                           const int height, bool top_down, int row_size) {
